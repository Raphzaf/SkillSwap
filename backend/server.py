...
# requests est optionnel (pour l'email via SendGrid)
try:
...
async def send_email_code(email: str, code: str):
    ...
    if SENDGRID_API_KEY and FROM_EMAIL and requests:
        try:
            resp = requests.post(
            ...
            logger.info("SendGrid status: %s", resp.status_code)
        except Exception:
...
async def delete_me(user=Depends(auth_user)):
    ...
    await db.auth_codes.delete_many({"email": {"$exists": True}})
